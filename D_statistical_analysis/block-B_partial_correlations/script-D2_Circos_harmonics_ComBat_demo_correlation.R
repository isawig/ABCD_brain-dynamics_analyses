...
##### 
# Load the dataset with harmonized FCH (ComBat)
df_fch_path <- "/Users/auroraberto/Desktop/24-25/MSc_thesis/main_article/neuroCombat_harmonization/results"
df_fch_file <- file.path(df_fch_path,"dataset_fch_harmonized.csv")
df_fch <- read.csv(df_fch_file, header = T, stringsAsFactors = T)
...
# Delete rows with NA
df_sub <- na.omit(df_sub) # 4399 x 46
...
plot_list <- list()
vars <- c("Harmonics_power14",  "Harmonics_power18",
          "Harmonics_energy14", "Harmonics_energy18")
